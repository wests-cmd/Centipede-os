...
import { ConnectionState, RuntimeStatus } from '../types';
import {
  Activity,
  AlertTriangle,
  Bot,
  ...
  return (
    <div className="flex flex-col h-screen bg-slate-950 text-slate-100 font-sans overflow-hidden select-none">
      ...
      <header className="h-12 bg-slate-900/90 border-b border-slate-800 px-4 flex items-center justify-between z-30 backdrop-blur-md">
        ...
        {/* Connection Status Badge & System Summary */}
        <div className="flex items-center space-x-4">
          ...
          <div
            ...
            className={`flex items-center space-x-2 px-3 py-1 rounded-full border text-xs font-semibold cursor-pointer transition-all ${
          ...
          >
            <span className={`w-2 h-2 rounded-full ${
              connectionState === 'online' ? 'bg-emerald-400' : connectionState === 'connecting' ? 'bg-amber-400' : 'bg-red-400'
            }`}></span>
      ...
      {connectionState === 'offline' && (
        <div className="bg-red-900/80 border-b border-red-700 text-red-100 px-4 py-2 flex items-center justify-between text-xs z-20">
          <div className="flex items-center space-x-2">
            <AlertTriangle className="w-4 h-4 text-red-300" />
            <span className="font-semibold">Kingdom Server Disconnected.</span>
            <span>Centipede OS is attempting automatic reconnection to {adapter.getBaseUrl()}...</span>
          </div>
          <button
            onClick={() => adapter.reconnect()}
            className="flex items-center space-x-1 bg-red-800 hover:bg-red-700 px-3 py-1 rounded text-white font-medium border border-red-600 transition-colors"
          >
      ...
      {/* Main OS View Workspace */}
      <main className="flex-1 overflow-auto bg-slate-950 p-2">
        {children}
      </main>

      {/* Bottom Desktop Dock / App Switcher Bar */}
