/**
 * Universal cross-runtime UTF-8 SHA-256 synchronous hash implementation.
 * Works identically in Browsers, Node.js, Bun, and Web Workers.
 * Properly processes multi-byte UTF-8 character strings.
 */
...
export function syncSha256(input: string | Uint8Array): string {
  ...
  const bitLenHi = Math.floor(byteLen / 0x20000000);
  const bitLenLo = (byteLen << 3) >>> 0;

  // Pre-padding length: byteLen + 1 (0x80) + 8 (bit count)
  const paddedLen = ((byteLen + 9 + 63) & ~63);
  const padded = new Uint8Array(paddedLen);
  padded.set(bytes);
  padded[byteLen] = 0x80;

  // Store 64-bit length at end of block (big-endian)
